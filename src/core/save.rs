...
use crate::gameplay::player::components::{
    AttackCooldown, AttackPower, CritChance, DashCooldown, Energy, Health, MoveSpeed, Player, RewardModifiers,
    RangedCooldown, Gold,
};
...
fn read_save_file() -> Result<SaveData> {
    ...
    let save = ron::from_str::<SaveData>(&content).with_context(|| format!("parse ron {}", path.display()))?;
    Ok(save)
}
...
fn apply_pending_load(
    ...
    *rewards = save.player.rewards;
    ...
    atk_cd.timer = Timer::from_seconds(save.player.attack_cd_s.max(0.05), TimerMode::Once);
    dash_cd.timer = Timer::from_seconds(save.player.dash_cd_s.max(0.05), TimerMode::Once);
    ranged_cd.timer = Timer::from_seconds(save.player.ranged_cd_s.max(0.05), TimerMode::Once);
